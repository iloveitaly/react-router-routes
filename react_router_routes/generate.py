...
def collect_route_patterns(routes: list[dict], parent_path: str = "") -> list[str]:
    patterns: list[str] = []
    for route in routes:
        # Clean up path by removing double slashes and trailing slashes
        current_path = (
            f"{parent_path}/{route.get('path', '')}".replace("//", "/").rstrip("/")
            or "/"
        )
...
Generated by app/lib/react_router_generate.py from the React Router config.
...
"""
from typing import Literal, overload, TypedDict, NotRequired, Mapping
import re
...
@overload
def react_router_path(path: Literal["{{ r.pattern }}"], params: {{ r.class_name }}, /) -> str: ...
@overload
def react_router_path(path: Literal["{{ r.pattern }}"], params: Mapping[str, object], /) -> str: ...
{% else %}
@overload
def react_router_path(path: Literal["{{ r.pattern }}"], params: None | Mapping[str, object] = None, /) -> str: ...
{% endif %}
{% endfor %}
@overload
...
{% if r.params or r.has_splat %}
@overload
def react_router_url(path: Literal["{{ r.pattern }}"], params: {{ r.class_name }}, base_url: str | None = None, /) -> str: ...
@overload
def react_router_url(path: Literal["{{ r.pattern }}"], params: Mapping[str, object], base_url: str | None = None, /) -> str: ...
{% else %}
@overload
def react_router_url(path: Literal["{{ r.pattern }}"], params: None | Mapping[str, object] = None, base_url: str | None = None, /) -> str: ...
{% endif %}
{% endfor %}
@overload
...
def react_router_url(path: RoutePaths, params: Mapping[str, object] | None = None, base_url: str | None = None) -> str:
    ...
    base = base_url if base_url is not None else config("BASE_URL", default=None, cast=str)
    if not base:
        log.error("base_url missing", path=path)
        return built
...
def generate_route_types(
    directory: Path = typer.Argument(
        ..., help="Path to React Router project directory"
    ),
