...
def test_url_params_functionality(tmp_path: Path) -> None:
    ...
    result = routes_typing.react_router_path(
        "/home", url_params={"page": "1", "sort": "name"}
    )
    assert result == "/home?page=1&sort=name"

    # Test url_params with react_router_url
    result = routes_typing.react_router_url(
        "/home", base_url="https://example.com", url_params={"foo": "bar"}
    )
    ...
    result = routes_typing.react_router_path(
        "/home", url_params={"query": "hello world", "special": "a&b"}
    )
